...
Divvy.Map = function() {
    ...
    this.initialize = function() {
        ...
        self.heatmap = new L.TileLayer.WebGLHeatMap({
            size: 600,
            autoresize: true,
            opacity: 0.5
        });
    ...
    this.addStationsToMap = function(collection, resp, xhr) {
        self.markers = self.markers || new L.FeatureGroup();
        collection.each(function(model) {
            ...
            self.addRollOverMarker(coords, message, self.markers)
            ...
            if (!(isNaN(lat) || isNaN(lng))) {
                ...
                self.heatmap.addDataPoint(lat, lng, Math.abs(attrs.availableBikes) * 10);
            }
            //self.map.addLayer(self.markers);
        });
    };

    this.addRollOverMarker = function(coords, message, markers) {
        var marker = new RollOverCircle(coords, 10, {
            color: '#2FCAFC',
            fillColor: '#2FCAFC',
            fillOpacity: 1.0
        });
